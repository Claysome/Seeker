...
class BinanceSpotHttp(object):
    ...
    def __init__(self, api_key=EX_BINANCE_KEY, secret=EX_BINANCE_SECRET, host=None, proxy_host=None, proxy_port=0, timeout=5, try_counts=5):
        self.logger = get_logger("gateway.binance.log")
        ...
        for i in range(0, self.try_counts):
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.logger.info(f"%s : %s" % (response.json(), response.status_code))
            except Exception as error:
                self.logger.info(f"请求:{path}, 发生了错误: {error}")
                time.sleep(3)
        return None
    ...
    def get_latest_price(self, pair: str):
        try:
            ...
            query_dict = {"symbol": pair}
            info = self.request(RequestMethod.GET, path, query_dict)
    ...
    def place_order(self, symbol: str, order_side: OrderSide, order_type: OrderType, quantity: float, price: float,
        ...
        try:
            ...
            info = self.request(RequestMethod.POST, path=path, requery_dict=params, verify=True)
        except Exception as ex:
            self.logger.error(repr(ex))
            return -1
